      ...
      <div style={{ minHeight: "100vh", display: "flex" }}>
        ...
        <div style={{ width: "45%", background: "linear-gradient(135deg, #0550CC 0%, #0866FF 55%, #3B82F6 100%)", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "3rem", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <h2 style={{ color: "white", fontSize: "2.2rem", fontWeight: 700, lineHeight: 1.2, marginBottom: "1rem" }}>Bienvenue sur<br />HostLink.</h2>
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.95rem", lineHeight: 1.7, marginBottom: "2rem" }}>Gerez vos biens, pilotez vos revenus et trouvez les meilleurs concierges depuis un seul endroit.</p>
            <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 14, padding: "1.25rem", border: "1px solid rgba(255,255,255,0.15)" }}>
              <p style={{ color: "rgba(255,255,255,0.9)", fontSize: "0.875rem", lineHeight: 1.6, marginBottom: "1rem", fontStyle: "italic" }}>"Depuis HostLink, mes 3 biens generent 38% de plus."</p>
          ...
          <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.75rem", position: "relative", zIndex: 1 }}>© 2025 HostLink</div>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 4rem" }}>
          {mode === "connexion" && (
            <div style={{ width: "100%", maxWidth: 420 }} className="fade">
              <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#111827", marginBottom: "0.4rem" }}>Connexion</h1>
              ...
              <button onClick={handleLogin} disabled={loading} style={{ width: "100%", padding: "0.9rem", background: "#0866FF", color: "white", border: "none", borderRadius: 10, fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: "0.95rem", cursor: loading?"not-allowed":"pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.6rem", opacity: loading?0.8:1, marginTop: "1rem" }}>
                {loading ? <><div style={{ width: 18, height: 18, border: "2px solid rgba(255,255,255,0.4)", borderTop: "2px solid white", borderRadius: "50%" }} className="spin" /> Connexion...</> : "Se connecter →"}
              </button>
            ...
            <div style={{ width: "100%", maxWidth: 420 }} className="fade">
              {!resetSent ? (
                <>
                  ...
                  <p style={{ color: "#6B7280", fontSize: "0.9rem", marginBottom: "2rem" }}>Entrez votre email pour recevoir un lien.</p>
                  <div style={{ marginBottom: "1.5rem" }}>
                    <label style={{ display: "block", fontSize: "0.82rem", fontWeight: 600, color: "#374151", marginBottom: "0.4rem" }}>Email</label>
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="jean@email.com" style={inputStyle} />
                  </div>
                  <button onClick={() => { if(email) setResetSent(true); }} style={{ width: "100%", padding: "0.9rem", background: "#0866FF", color: "white", border: "none", borderRadius: 10, fontFamily: "Inter, sans-serif", fontWeight: 700, cursor: "pointer" }}>
                    Envoyer le lien →
                  </button>
                </>
              ) : (
                <div style={{ textAlign: "center" }} className="fade">
                  ...
                  <p style={{ color: "#6B7280", fontSize: "0.875rem", lineHeight: 1.7, marginBottom: "2rem" }}>Verifiez votre boite mail et cliquez sur le lien.</p>
                  <button onClick={() => { setMode("connexion"); setResetSent(false); }} style={{ width: "100%", padding: "0.9rem", background: "#0866FF", color: "white", border: "none", borderRadius: 10, fontFamily: "Inter, sans-serif", fontWeight: 700, cursor: "pointer" }}>
